...
const Dashboard = () => {
  ...
  const calculateDashboard = (
    ...
    const lowStock = allProducts.filter(
      (p) => (parseInt(p.stock_quantity) || 0) < 10
    );
    ...
    const rankedProducts = Object.keys(productSalesCount)
      .map((pid) => {
    ...
    allCategories.forEach(
      (cat) => (categoryNameMap[cat.id] = cat.category_name)
    );
    const catCountMap = {};
    allProducts.forEach((p) => {
      let catName = "ไม่ระบุหมวดหมู่";
      if (p.Categories_category_id && categoryNameMap[p.Categories_category_id])
        catName = categoryNameMap[p.Categories_category_id];
      else if (p.category) catName = p.category;
      catCountMap[catName] = (catCountMap[catName] || 0) + 1;
    });
    const totalProds = allProducts.length || 1;
    const chartData = Object.keys(catCountMap)
      .map((key) => ({
        name: key,
        value: catCountMap[key],
        percent: ((catCountMap[key] / totalProds) * 100).toFixed(1),
      }))
    ...
    orders.forEach((order) => {
      if (order.transaction_date) {
        ...
        if (salesMap[dateStr] !== undefined)
          salesMap[dateStr] += Number(order.grand_total) || 0;
      }
    });
  ...
  const filteredTableData = recentOrders.filter((order) => {
    ...
    return (
      orderNo.includes(term) || status.includes(term) || payment.includes(term)
    );
  });
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredList.map((order) => {
      const dateObj = order.transaction_date.seconds
        ? new Date(order.transaction_date.seconds * 1000)
        : new Date(order.transaction_date);
      ...
      }","ลูกค้าทั่วไป","${order.grand_total}","${
        order.payment_method === "cash" ? "เงินสด" : "โอนเงิน"
      }","${order.status}"`;
    });
    ...
    link.setAttribute(
      "download",
      `ยอดขาย_${dayjs(startDate).format("DD-MM-YYYY")}_ถึง_${dayjs(
        endDate
          ...
          >
            <Box>
              ...
              <Typography
                ...
                sx={{ mt: 0.5 }}
              >
                ภาพรวมธุรกิจและการวิเคราะห์ยอดขาย
              </Typography>
            </Box>
                ...
                <TableContainer sx={{ flex: 1, maxHeight: 600 }}>
                  <Table stickyHeader size="small">
                    <TableHead sx={{ "& th": { bgcolor: "#F9FAFB" } }}>
                      <TableRow>
                        <TableCell sx={{ pl: 3, fontWeight: 700 }}>
                          เลขที่บิล
                        </TableCell>
                    ...
                    <TableBody>
                      {paginatedData.map((order) => (
                        <TableRow
                          key={order.id}
                          hover
                          sx={{ "& td": { borderBottom: "1px solid #F2F4F7" } }}
                        >
                          ...
                          <TableCell>
                            <Typography variant="body2">
                              ...
                              order.transaction_date.seconds
                                ? new Date(
                                    order.transaction_date.seconds * 1000
                ...
                <List sx={{ py: 0 }}>
                  ...
                  {topProducts.length === 0 && (
                    <Box p={3} textAlign="center">
                      <Typography variant="caption">ไม่มีข้อมูล</Typography>
                    </Box>
                  )}
                </List>
              </Card>
            ...
            <DialogContent sx={{ p: 0 }}>
              ...
              >
                <Box textAlign="center" mb={2}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {selectedOrder?.transaction_date?.seconds
                      ? new Date(
                          selectedOrder.transaction_date.seconds * 1000
                ...
                <Box textAlign="center" mb={2}>
                  ...
                  <Typography variant="caption" display="block">
                    เลขที่ใบเสร็จ: {selectedOrder?.order_number}
                  </Typography>
                  <Typography variant="caption" display="block">
                    พนักงาน: {selectedOrder?.cashier_name || "Admin"}
                  </Typography>
                </Box>
                ...
                <Box display="flex" justifyContent="space-between">
                  ...
                  <Typography variant="caption">
                    {selectedOrder?.received_amount?.toLocaleString()}
                  </Typography>
                </Box>
                <Box display="flex" justifyContent="space-between">
